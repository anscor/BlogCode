...
// 右旋转
AVLTree::AVLTreeNode* AVLTree::RotateLL(AVLTreeNode* root) {
    AVLTree::AVLTreeNode* temp = root->left;
    root->left = temp->right;
    temp->right = root;

    // 旋转后root与temp节点高度发生了变化，需重新计算
    CalcHeight(temp);
    CalcHeight(root);
    return temp;
}
...
// 左旋转
AVLTree::AVLTreeNode* AVLTree::RotateRR(AVLTreeNode* root) {
...
int AVLTree::Find(int val) {
    AVLTreeNode* temp = root;
    while (temp) {
        // 如果当前节点为要查找的值，返回1表示该值存在，查找成功
        if (temp->val == val) return 1;
        // 如果当前节点值小于要查找的值，则表示要查找的值位于右子树上，查找右子树
        if (temp->val < val)
            temp = temp->right;
        // 如果当前节点值大于要查找的值，则表示要查找的值位于左子树上，查找左子树
        else
            temp = temp->left;
    }
